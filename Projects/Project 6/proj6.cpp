...
int main() {
  ...
  cout << "Testing VIRTUAL Move Function for DERIVED Class Objects"
  << endl;
  float lla_new[3] = {37.77, 122.42, 52.0};
  c1.move( lla_new );

  cout << "Testing Insertion operator<< Overload for BASE Class Objects"
  << endl;
  cout << c_rno << endl;
  ...
  cout << "Testing VIRTUAL Move Function for Base Class Pointers" << endl;
  for(int i = 0; i < 3; ++i) {
    vehicles_array[i]->move( lla_new );
  }

  cout << "Testing Insertion operator<< Overload for Base Class Pointers"
  << endl;
  for(int i = 0; i < 3; ++i) {
